...
namespace MyMediaLite.DataType
{
	...
	public class SparseMatrix<T> : IMatrix<T> where T:new()
	{
		...
		public virtual Dictionary<int, T> this [int x]
		{
			get {
				Dictionary<int,T> tmp = new Dictionary<int, T> ();
				int i = 0;
				foreach(int y in index_list[x]){
					tmp.Add(y, this[x,y]);
					i++;
				}
				return tmp;
		...
		public virtual void setTotalRow(int x, IList<T> row){
			value_list [x] = (List<T>)row;
		}

		public virtual IList<T> getTotalRow(int x){
			return value_list [x];
//			IList<T> tmp = new List<T> ();
//			for (int y = 0; y <this.index_list[x].Count; y++){
//				tmp.Add (this [x, y]);
//			}
//			List<IList<T>> bla = new List<IList<T>> ();
//			bla.Add (tmp);
//			bla.Add (value_list [x]);
//			return bla;
