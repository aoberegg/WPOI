...
namespace MyMediaLite.DataType
{
	...
	public static class SparseMatrixExtensions
	{
		...
		static public void print(this SparseMatrix<float> matrix)
		{
			int old_item2 = 0;
			bool write_id = true;
			foreach (Tuple<int,int> tuple in matrix.NonEmptyEntryIDs){
				if (write_id) {
					Console.WriteLine ();
				...
				if (old_item2 > tuple.Item2) {
					write_id = true;
				}
				
				Console.Write (matrix [tuple.Item1, tuple.Item2]);
				Console.Write (";");
				old_item2 = tuple.Item2;
			}
			Console.WriteLine ();
		}
	}
